...
### 🔻 Example 2 — Skill Matching for a Job
```python
required_skills = {"python", "sql", "git", "django", "postgresql"}

applicant_ali  = {"python", "sql", "git", "excel", "powerbi"}
applicant_sara = {"python", "sql", "git", "django", "postgresql", "docker"}

def evaluate(name, applicant, required):
    matched = applicant & required
    missing = required - applicant
    extra   = applicant - required
...
evaluate("Ali",  applicant_ali,  required_skills)
evaluate("Sara", applicant_sara, required_skills)
...
### 🔻 Example 3 — Social Media Mutual Friends
```python
friends = {
    "Ali":   {"Sara", "Bilal", "Hina", "Omar"},
    "Sara":  {"Ali", "Bilal", "Zara", "Nadia"},
    "Bilal": {"Ali", "Sara", "Omar", "Zara"},
}

def mutual_friends(person_a, person_b):
    return friends[person_a] & friends[person_b]

def suggestions(person):
    all_friends_of_friends = set()
    for friend in friends[person]:
        if friend in friends:
            all_friends_of_friends |= friends[friend]
    return all_friends_of_friends - friends[person] - {person}

print("Ali & Sara mutual friends :", mutual_friends("Ali", "Sara"))
print("Suggestions for Ali       :", suggestions("Ali"))
